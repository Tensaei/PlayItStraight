...
def plain_training(model, target_accuracy, incremental_training_epochs, criterion, optimizer, n_samples_at_start, dataset):
...
def cutted_training(model, training_epochs, criterion, optimizer, n_samples_at_start, n_samples_to_select, al_dataset, incremental_technique, target_accuracy, selection_policy):
    clprint("Loading dataset...", Reason.INFO_TRAINING)
    ...
    active_learner = SimpleActiveLearner(al_dataset, incremental_technique, target_accuracy, selection_policy)
    start_time = support.get_time_in_millis()
...
if __name__ == "__main__":
    ...
    if not skip_classic_training:
        ...
        clprint("Plain training configuration -> target accuracy: {}, n_samples: {}".format(target_accuracy, plain_n_samples_at_start), Reason.INFO_TRAINING, loggable=True)
        clprint("Samples used: {}".format(plain_n_samples_at_start), Reason.INFO_TRAINING, loggable=True)
        dataset_class, model, criterion, optimizer = load_data_and_model(sys.argv[1], plain_n_samples_at_start)
        total_epochs = plain_training(model, target_accuracy, incremental_training_epochs, criterion, optimizer, plain_n_samples_at_start, dataset_class)
        clprint("CPU usage: {}\nMemory usage: {}\nElapsed time: {}\nEpochs elapsed: {}".format((sum_cpu_percent/counts), (sum_memory_usage/counts), (support.get_time_in_millis() - start_time), total_epochs), Reason.OTHER, loggable=True)
    ...
    clprint("-" * 100, Reason.INFO_TRAINING, loggable=True)
    clprint("Incremental training configuration -> epochs: {}, n_start_samples: {}, n_select_samples: {}, target_accuracy: {}".format(incremental_training_epochs, incremental_n_samples_at_start, incremental_n_samples_to_select, target_accuracy), Reason.INFO_TRAINING, loggable=True)
    ...
    total_epochs = cutted_training(model, incremental_training_epochs, criterion, optimizer, incremental_n_samples_at_start, incremental_n_samples_to_select, dataset, incremental_technique, target_accuracy, selection_policy)
    clprint("Samples used: {}".format((incremental_n_samples_at_start + (incremental_n_samples_to_select * total_epochs))), Reason.INFO_TRAINING, loggable=True)
