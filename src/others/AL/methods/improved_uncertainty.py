...
class ImprovedUncertainty(ALMethod):
    ...
    def rank_uncertainty(self, index=None):
        ...
        with (torch.no_grad()):
            ...
            # train_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(self.dst_unlabeled, indices), batch_size=self.args.test_batch_size, num_workers=self.args.workers)
